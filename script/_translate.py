...
def main():
	D='video';E='C:/Users/Xiesx/Desktop/test/test.mp4';I=_A;F='zh-cn';G=_B;H=False;B=argparse.ArgumentParser(description='Video Translation');B.add_argument('--url','-u',help='Video URL');B.add_argument('--input','-i',help='Input path of the video to be translated');B.add_argument('--output','-o',default=D,help='Output path of the translated video');B.add_argument('--language','-l',default=F,help='Subtitle translation language');B.add_argument('--voice','-v',default=G,help='Voice for subtitle dubbing');B.add_argument('--voice_volume','-vv',type=float,default=.8,help='Volume of the dubbed voice');B.add_argument('--background_volume','-bv',type=float,default=.2,help='Volume of the background music');B.add_argument('--model','-m',default=WHISPER_MODEL,help='Transcription model');B.add_argument('--debug','-d',action='store_true',default=H,help='Enable debug mode');A=B.parse_args()
	if A.debug:logger.info('Debug mode enabled');C=E
	if A.input:C=A.input
