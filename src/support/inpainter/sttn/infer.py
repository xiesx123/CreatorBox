...
from src.support.inpainter.sttn.sttn import InpaintGenerator
from src.support.inpainter.sttn.utils import Stack, ToTorchFormatTensor

# 设备
DEVICE = "cpu"
# 模型
STTN_MODEL_PATH = "D:/Projects/fast/fast-creator/models/torch/hub/checkpoints/infer_model.pth"
...
class STTNInpaint:
    ...
    def __init__(self):
        ...
        self.model.load_state_dict(torch.load(STTN_MODEL_PATH, map_location="cpu")["netG"])
        # 3. # 将模型设置为评估模式
        self.model.eval()
        # 模型输入用的宽和高
...
class STTNVideoInpaint:
    ...
    def read_frame_info_from_video(self):
        ...
        frame_info = {"W_ori": int(reader.get(cv2.CAP_PROP_FRAME_WIDTH) + 0.5), "H_ori": int(reader.get(cv2.CAP_PROP_FRAME_HEIGHT) + 0.5), "fps": reader.get(cv2.CAP_PROP_FPS), "len": int(reader.get(cv2.CAP_PROP_FRAME_COUNT) + 0.5)}  # 视频的原始宽度  # 视频的原始高度  # 视频的帧率  # 视频的总帧数
        # 返回视频读取对象、帧信息和视频写入对象
    ...
    def __init__(self, video_path, mask_path=None, clip_gap=None):
        ...
        # 设置输出视频文件的路径
        self.video_out_path = os.path.join(os.path.dirname(os.path.abspath(self.video_path)), f"{os.path.basename(self.video_path).rsplit('.', 1)[0]}_no_sub.mp4")
        # 配置可在一次处理中加载的最大帧数
        if clip_gap is None:
            self.clip_gap = STTN_MAX_LOAD_NUM
        else:
...
if __name__ == "__main__":
    ...
    inpainter = STTNVideoInpaint(
        video_path="C:/Users/xiesx/Desktop/video_dinner.mp4",
        mask_path="C:/Users/xiesx/Desktop/vmi/video_dinner/mask/00000.png",
    )
    inpainter()
