...
sys.path.append("src/support/third_party/gptsovits")
sys.path.append("src/support/third_party/gptsovits/GPT_SoVITS")
...
def run(rank, n_gpus, hps):
    ...
    net_d = MultiPeriodDiscriminator(hps.model.use_spectral_norm, version=hps.model.version).cuda(rank) if torch.cuda.is_available() else MultiPeriodDiscriminator(hps.model.use_spectral_norm, version=hps.model.version).to(device)
    ...
    try:  # 如果能加载自动resume
        ...
        if hps.train.pretrained_s2D != "" and hps.train.pretrained_s2D != None and os.path.exists(hps.train.pretrained_s2D):
            ...
            print(
                ...
                (
                    net_d.module.load_state_dict(torch.load(hps.train.pretrained_s2D, map_location="cpu", weights_only=False)["weight"], strict=False)
                    if torch.cuda.is_available()
