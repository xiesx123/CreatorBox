...
class TTS_Config:
    ...
    def __init__(self, config: Union[dict, str] = "", version: str = "v2"):
        if config in ["", None]:
            ...
            self.configs_path: str = os.path.join(os.environ.get('sovits'), configs_base_path, "tts_infer.yaml")
        ...
        self.t2s_weights_path = os.path.join(os.environ.get("sovits"), self.configs.get("t2s_weights_path", None))
        self.vits_weights_path = os.path.join(os.environ.get("sovits"), self.configs.get("vits_weights_path", None))
        self.bert_base_path = os.path.join(os.environ.get("sovits"), self.configs.get("bert_base_path", None))
        self.cnhuhbert_base_path = os.path.join(os.environ.get("sovits"), self.configs.get("cnhuhbert_base_path", None))
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if "Pro" in model_version:
            self.init_sv_model()
        path_sovits = os.path.join(os.environ.get('sovits'),self.configs.default_configs[model_version]["vits_weights_path"])
    ...
    def init_vocoder(self, version: str):
        if version == "v3":
            ...
            self.vocoder = BigVGAN.from_pretrained(
                "%s/GPT_SoVITS/pretrained_models/models--nvidia--bigvgan_v2_24khz_100band_256x" % (os.environ.get('sovits'),),
            ...
            state_dict_g = torch.load(
                "%s/GPT_SoVITS/pretrained_models/gsv-v4-pretrained/vocoder.pth" % (os.environ.get('sovits'),),
                map_location="cpu",
                weights_only=False,
            )
