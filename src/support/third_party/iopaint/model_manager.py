...
from iopaint.model import SDXL, models
...
from iopaint.model.controlnet import ControlNet
from iopaint.model.power_paint.power_paint_v2 import PowerPaintV2
from iopaint.model.sd import SD
from iopaint.model.utils import is_local_files_only, torch_gc
...
class ModelManager:
    def __init__(self, name: str, device: torch.device, **kwargs):
        ...
        if controlnet_method is None and name in self.available_models and self.available_models[name].support_controlnet:
    ...
    def init_model(self, name: str, device, **kwargs):
        ...
        if name not in self.available_models:
            raise NotImplementedError(f"Unsupported model: {name}. Available models: {list(self.available_models.keys())}")
    ...
    def switch(self, new_name: str):
        ...
        if self.available_models[new_name].support_controlnet and self.controlnet_method not in self.available_models[new_name].controlnets:
        ...
        try:
            ...
            self.model = self.init_model(new_name, switch_mps_device(new_name, self.device), **self.kwargs)
        except Exception as e:
            self.name = old_name
            self.controlnet_method = old_controlnet_method
            logger.info(f"Switch model from {old_name} to {new_name} failed, rollback")
            self.model = self.init_model(old_name, switch_mps_device(old_name, self.device), **self.kwargs)
            raise e
    ...
    def switch_brushnet_method(self, config):
        ...
        if self.enable_brushnet and config.brushnet_method and self.brushnet_method != config.brushnet_method:
            ...
            logger.info(f"Switch Brushnet method from {old_brushnet_method} to {config.brushnet_method}")
    ...
    def switch_controlnet_method(self, config):
        ...
        if self.enable_controlnet and config.controlnet_method and self.controlnet_method != config.controlnet_method:
            old_controlnet_method = self.controlnet_method
            self.controlnet_method = config.controlnet_method
            self.model.switch_controlnet_method(config.controlnet_method)
            logger.info(f"Switch Controlnet method from {old_controlnet_method} to {config.controlnet_method}")
