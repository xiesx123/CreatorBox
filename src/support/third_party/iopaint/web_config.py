        ...
        with gr.Tabs():
            with gr.Tab("Common"):
                ...
                with gr.Column():
                    model = gr.Textbox(
                        init_config.model,
                        label="Current Model. Model will be automatically downloaded. " "You can select a model in Recommended Models or Downloaded Models or manually enter the SD/SDXL model ID from HuggingFace, for example, runwayml/stable-diffusion-inpainting.",
                    )
                ...
                cpu_textencoder = gr.Checkbox(init_config.cpu_textencoder, label=f"{CPU_TEXTENCODER_HELP}")
                disable_nsfw_checker = gr.Checkbox(init_config.disable_nsfw_checker, label=f"{DISABLE_NSFW_HELP}")
            ...
            with gr.Tab("Plugins"):
                with gr.Row():
                    enable_interactive_seg = gr.Checkbox(init_config.enable_interactive_seg, label=INTERACTIVE_SEG_HELP)
                ...
                with gr.Row():
                    enable_remove_bg = gr.Checkbox(init_config.enable_remove_bg, label=REMOVE_BG_HELP)
                ...
                with gr.Row():
                    enable_anime_seg = gr.Checkbox(init_config.enable_anime_seg, label=ANIMESEG_HELP)

                with gr.Row():
                    enable_realesrgan = gr.Checkbox(init_config.enable_realesrgan, label=REALESRGAN_HELP)
                ...
                with gr.Row():
                    enable_restoreformer = gr.Checkbox(init_config.enable_restoreformer, label=RESTOREFORMER_HELP)
