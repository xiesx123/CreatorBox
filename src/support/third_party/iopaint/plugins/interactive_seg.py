...
from iopaint.plugins.segment_anything.build_sam import sam_model_registry
from iopaint.plugins.segment_anything.predictor import SamPredictor
...
class InteractiveSeg(BasePlugin):
    ...
    def _init_session(self, model_name: str):
        ...
        if "sam_hq" in model_name:
            self.predictor = SamHQPredictor(sam_model_registry[model_name](checkpoint=model_path).to(self.device))
        elif model_name.startswith("sam2"):
            sam2_model = build_sam2(model_name, ckpt_path=model_path, device=self.device)
            self.predictor = SAM2ImagePredictor(sam2_model)
        else:
            self.predictor = SamPredictor(sam_model_registry[model_name](checkpoint=model_path).to(self.device))
