...
sys.path.append("src/support/third_party/gpt_sovits/GPT_SoVITS")
...
language = os.environ.get("language", "en_US")
...
class TTS_Config:
    ...
    def __init__(self, config: Union[dict, str] = "", version: str = "v2"):
        if config in ["", None]:
            # 设置默认配置文件路径
        ...
        # 修改实例默认配置（把新的给到default_configs）
        self.default_configs = deepcopy(configs_)
        # 获取自定义实例化配置（把新的给到default_configs）
        # self.configs: dict = configs_.get("custom", configs_[version])
        self.configs: dict = configs_[version]
        ...
        self.t2s_weights_path = os.path.abspath(os.path.join(os.environ.get("sovits"), self.configs.get("t2s_weights_path", None)))
        os.environ["t2s_weights_path"] = self.t2s_weights_path
        self.vits_weights_path = os.path.abspath(os.path.join(os.environ.get("sovits"), self.configs.get("vits_weights_path", None)))
        os.environ["vits_weights_path"] = self.vits_weights_path
        self.bert_base_path = os.path.abspath(os.path.join(os.environ.get("sovits"), self.configs.get("bert_base_path", None)))
        os.environ["bert_base_path"] = self.bert_base_path
        self.cnhuhbert_base_path = os.path.abspath(os.path.join(os.environ.get("sovits"), self.configs.get("cnhuhbert_base_path", None)))
        os.environ["cnhuhbert_base_path"] = self.cnhuhbert_base_path
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        if if_lora_v3 == False:
            logger.info(f"Loading VITS weights from {weights_path}. {vits_model.load_state_dict(dict_s2['weight'], strict=False)}")
        else:
            logger.info(f"Loading VITS pretrained weights from {weights_path}. {vits_model.load_state_dict(load_sovits_new(path_sovits)['weight'], strict=False)}")
            ...
            logger.info(f"Loading LoRA weights from {weights_path}. {vits_model.load_state_dict(dict_s2['weight'], strict=False)}")
    ...
    def init_vocoder(self, version: str):
        if version == "v3":
            ...
            self.vocoder.remove_weight_norm()
            state_dict_g = torch.load(f"{os.environ.get('sovits')}/gsv-v4-pretrained/vocoder.pth", map_location="cpu", weights_only=False)
