...
from src.support.tracker.model.aux_modules import AuxComputer
from src.support.tracker.model.big_modules import (
    KeyProjection,
    MaskDecoder,
    MaskEncoder,
    PixelEncoder,
...
from src.support.tracker.model.group_modules import *
from src.support.tracker.model.modules import *
from src.support.tracker.model.transformer.object_summarizer import ObjectSummarizer
from src.support.tracker.model.transformer.object_transformer import QueryTransformer
from src.support.tracker.model.utils.memory_utils import *
from src.support.tracker.utils.tensor_utils import aggregate
...
class CUTIE(nn.Module):
    ...
    def encode_image(self, image: torch.Tensor) -> (Iterable[torch.Tensor], torch.Tensor):  # type: ignore
        image = (image - self.pixel_mean) / self.pixel_std
        ms_image_feat = self.pixel_encoder(image)
        return ms_image_feat, self.pix_feat_proj(ms_image_feat[0])

    def encode_mask(self, image: torch.Tensor, ms_features: List[torch.Tensor], sensory: torch.Tensor, masks: torch.Tensor, *, deep_update: bool = True, chunk_size: int = -1, need_weights: bool = False) -> (torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor):  # type: ignore
        image = (image - self.pixel_mean) / self.pixel_std
    ...
    def transform_key(self, final_pix_feat: torch.Tensor, *, need_sk: bool = True, need_ek: bool = True) -> (torch.Tensor, torch.Tensor, torch.Tensor):  # type: ignore
    ...
    def readout_query(self, pixel_readout, obj_memory, *, selector=None, need_weights=False) -> (torch.Tensor, Dict[str, torch.Tensor]):  # type: ignore
        return self.object_transformer(pixel_readout, obj_memory, selector=selector, need_weights=need_weights)

    def segment(self, ms_image_feat: List[torch.Tensor], memory_readout: torch.Tensor, sensory: torch.Tensor, *, selector: bool = None, chunk_size: int = -1, update_sensory: bool = True) -> (torch.Tensor, torch.Tensor, torch.Tensor):  # type: ignore
        """
