...
from src.support.tracker.inference.image_feature_store import ImageFeatureStore
from src.support.tracker.inference.memory_manager import MemoryManager
from src.support.tracker.inference.object_manager import ObjectManager
from src.support.tracker.model.cutie import CUTIE
from src.support.tracker.utils.tensor_utils import aggregate, pad_divide_by, unpad
...
class InferenceCore:
    ...
    def _add_memory(self, image: torch.Tensor, pix_feat: torch.Tensor, prob: torch.Tensor, key: torch.Tensor, shrinkage: torch.Tensor, selection: torch.Tensor, *, is_deep_update: bool = True, force_permanent: bool = False) -> None:
        """
        ...
        msk_value, sensory, obj_value, self.obj_logits = self.network.encode_mask(image, pix_feat, self.memory.get_sensory(self.object_manager.all_obj_ids), prob, deep_update=is_deep_update, chunk_size=self.chunk_size, need_weights=self.save_aux)
        self.memory.add_memory(key, shrinkage, msk_value, obj_value, self.object_manager.all_obj_ids, selection=selection, as_permanent=as_permanent)
        self.last_mem_ti = self.curr_ti
        if is_deep_update:
            self.memory.update_sensory(sensory, self.object_manager.all_obj_ids)

    def _segment(self, key: torch.Tensor, selection: torch.Tensor, pix_feat: torch.Tensor, ms_features: Iterable[torch.Tensor], update_sensory: bool = True) -> torch.Tensor:
        """
        ...
        sensory, _, pred_prob_with_bg = self.network.segment(ms_features, memory_readout, self.memory.get_sensory(self.object_manager.all_obj_ids), chunk_size=self.chunk_size, update_sensory=update_sensory)
        # remove batch dim
        if self.flip_aug:
    ...
    def step(self, image: torch.Tensor, mask: Optional[torch.Tensor] = None, objects: Optional[List[int]] = None, *, idx_mask: bool = True, end: bool = False, delete_buffer: bool = True, force_permanent: bool = False) -> torch.Tensor:
        """
        ...
        if self.max_internal_size > 0:
            ...
            if min_side > self.max_internal_size:
                ...
                if mask is not None:
                    if idx_mask:
                        mask = F.interpolate(mask.unsqueeze(0).unsqueeze(0).float(), size=(new_h, new_w), mode="nearest", align_corners=False)[0, 0].round().long()
                    else:
