...
class ImageFeatureStore:
    ...
    def get_features(self, index: int, image: torch.Tensor) -> (Iterable[torch.Tensor], torch.Tensor):  # type: ignore
        if index not in self._store:
            self._encode_feature(index, image)

        return self._store[index][:2]

    def get_key(self, index: int, image: torch.Tensor) -> (torch.Tensor, torch.Tensor, torch.Tensor):  # type: ignore
        if index not in self._store:
            self._encode_feature(index, image)
