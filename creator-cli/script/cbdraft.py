...
def jianying_draft_content(info,audio_info,video_info,output):
	...
	A.dump(output)
def main():
	C=argparse.ArgumentParser(description='合成视频和音频');C.add_argument('--input',type=str,required=True,help='Input JSON file path');C.add_argument('--output',type=str,required=True,help='Output JSON file path');A=C.parse_args()
	try:
		with open(A.input,'r',encoding='utf-8')as H:D=json.load(H)
	except Exception as E:logger.error(f"Error reading JSON: {E}");return-2
	I=D.get('info');B=D.get('audio');F=D.get('video')
	if B and not os.path.exists(B.get(_A)):logger.error(f"Audio file {B.get(_A)} does not exist");return-2
	for G in F.get(_B,[]):
		if not os.path.exists(G.get(_A)):logger.error(f"Video file {G.get(_A)} does not exist");return-2
	try:jianying_draft_content(I,B,F,A.output);print(f"Generation successful, output path: {A.output}")
	except Exception as E:logger.error(f"Error during generation: {E}");return-2
