_H='Enable debug mode'
_G='--debug'
_F='--background-volume'
_E='--input'
_D='zh-CN-YunjianNeural'
_C=False
_B=None
_A=True
import os,sys
...
def merge_video(video_json_path,video_volume,audio_background_path,audio_background_volume=1.,duration=_B,debug=_C):
	...
	if B:cbutils.copy_file(E,'video')
	return E
async def translate_video(video_path,srt_path,target_language='zh-CN',target_voice=_D,audio_vocals_volume=.8,audio_background_volume=.2,model=str,debug=_C):
...
@cli.command()
@click.option(_E,'-i',type=click.Path(exists=_A),help='Collection of video clips')
...
@click.option(_F,'-bv',type=float,default=.5,show_default=_A,help='Background music volume (default: 1.0)')
@click.option('--duration','-d',type=int,help='Duration of the background music')
@click.option(_G,is_flag=_C,show_default=_A,help=_H)
